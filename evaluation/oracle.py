def compute_oracle_stats(blocks: list[Block], codec_registry: dict) -> dict:
    ...
    for block, codec_id in zip(blocks, oracle_actions):
        ...
        ratio = len(compressed) / original_size if original_size > 0 else 0.0
    ...
    mean_oracle_ratio = sum(per_block_best_ratio) / len(per_block_best_ratio) if per_block_best_ratio else 0.0
    return {
