...
_HEADER_FORMAT = "<4sIII"  # magic:4s, version:uint32, n_blocks:uint32, block_size:uint32
_HEADER_SIZE = struct.calcsize(_HEADER_FORMAT)
_VERSION = 1

def write_file_header(f: BinaryIO, n_blocks: int, block_size: int) -> None:
    header = struct.pack(_HEADER_FORMAT, ORBIT_MAGIC, _VERSION, n_blocks, block_size)
    f.write(header)

def read_file_header(f: BinaryIO) -> ORBITHeader:
    header_bytes = f.read(_HEADER_SIZE)
    magic, version, n_blocks, block_size = struct.unpack(_HEADER_FORMAT, header_bytes)
    return ORBITHeader(magic=magic, version=version, n_blocks=n_blocks, block_size=block_size)
